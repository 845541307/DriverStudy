#include <ntddk.h>
#define		DEVICE_NAME				TEXT("\\Device\\MyStringDevice")
#define		DEVICE_SYMLINKNAME		TEXT("\\??\\StringDevice")
...
{
	{
		UNICODE_STRING str;
		WCHAR *pwc = MYSTRING;
		RtlInitUnicodeString(&str, pwc);
		KdPrint(("why::str is %wZ\n", &str));
		//改变pwc
		try
		{
	...
	
	{
		UNICODE_STRING str;
		WCHAR *pwc = MYSTRING;
		str.Buffer = (PWCHAR)ExAllocatePool(PagedPool, BUFFER_SIZE);
		str.MaximumLength = BUFFER_SIZE;
		str.Length = 2 * wcslen(pwc);
		//保证缓冲区足够大，否则程序终止
		ASSERT(str.MaximumLength >= str.Length);
		RtlCopyMemory(str.Buffer, pwc, str.Length);
		str.Length = 2 * wcslen(pwc);
		KdPrint(("why::unicode str is %wZ\n", &str));
