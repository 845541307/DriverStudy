...
NTSTATUS DeAttach(PDEVICE_OBJECT pdevice)
{
	...
	{
		IoDeleteDevice(devExt->FilterDevice);
...
NTSTATUS DriverUnload(PDRIVER_OBJECT pDriver)
{
	...
	//delay some time
	lDelay = RtlConvertLongToLargeInteger(10 * DELAY_ONE_SECOND);
	...
	while (pDevice)
	{
		KdPrint(("why::DriverUnload DeAttach\n"));
		DeAttach(pDevice);
	...
	while (g_KeyCount > 0)
	{
		KdPrint(("why::wait key\n"));
		KeDelayExecutionThread(KernelMode, FALSE, &lDelay);
	}
	KdPrint(("why::DriverUnload success\n"));
	return STATUS_SUCCESS;
}
...
NTSTATUS ReadComp(PDEVICE_OBJECT pDevice, PIRP pIrp, PVOID Context)
{
	...
	if (NT_SUCCESS(pIrp->IoStatus.Status))
	{
		...
		{
			KdPrint(("why::numkey:%u\n", keyNumber));
			KdPrint(("why::sancode:%x\n", myData->MakeCode));
	...
	KdPrint(("why::ReadComp\n"));
	g_KeyCount--;
	if (pIrp->PendingReturned)
	{
		//KdPrint(("why::IoMarkIrpPending\n"));
...
NTSTATUS AttachDevice(PDRIVER_OBJECT pDriver, PUNICODE_STRING RegPatch)
{
	...
	while (targetDevice)
	{
		...
		KdPrint(("why::Attach success!\n"));
	}
	DbgPrint("why::Create And Attach Finshed...\n");
	return status;
}
